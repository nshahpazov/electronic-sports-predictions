...
team_heroes_df %>%
  left_join(
    team_heroes_ratios_df, by = c("hero_id_team_1", "hero_id_team_2", "account_id")
  ) %>%
  arrange(match_id) %>%
  mutate(n = c(rep(seq(1, 5), 50000 - 1))) %>%
...
as.data.frame(map(match_data, ~sum(is.na(.x))))
```
...
compare_models <- function (model1, model2, metric = model1$metric[1], ...) {
  model_difference <- list(model1, model2) %>%
    resamples() %>%
...
# make an example in order to understand it better
...
null_model <- train(
  ...
  preProc = "YeoJohnson", # preprocessing of the predictors
  method = "glm",
  metric = "ROC",
  trControl = ctrl
)
...
features_to_try <- c(
  colnames(select(matches_player_rankings_df, -match_id)),
  colnames(select(matches_hero_stats_df, -match_id))
)
...
for (i in 1:nrow(one_predictor_results)) {
  ...
  one_predictor_results$ROC[i] <- getTrainPerf(var_mod)[1, "TrainROC"]
