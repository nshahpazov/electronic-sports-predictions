...
for (i in 1:length(stat_cols)) {
  matches_hero_stats_df[stat_cols[i]] <- diff_means(
    matches_hero_stats_df,
    glue("{stat_cols[i]}_[0-4]$"),
    glue("{stat_cols[i]}_1[2-3]")
  )
}
...
compare_models <- function (model1, model2, metric = model1$metric[1], ...) {
  glm_resampels <- list(model1, model2) %>%
    resamples()

  model_difference <- glm_rescamples %>%
