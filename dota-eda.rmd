...
```{r frequency-generation-playground}
m <- matrix(sample(1:5, 25, replace = TRUE), nrow = 5)
freq <- matrix(0, 5, 5)
game_lengths <- sample(1:10, 5) # game lengths

freq[cbind(m[,1], m[,2])] <- freq[cbind(m[,1], m[,2])] + 1
freq[cbind(m[,2], m[,3])] <- freq[cbind(m[,2], m[,3])] + 1

# this approach would not work, have to rewrite it in a better way

# some standard moving through states
m <- matrix(rep(1:5, 5), byrow = TRUE, nrow = 5, ncol = 5)

len <- 5 # lengths we are going to go to

for (i in 1:nrow(m)) {
  for (j in 1:len - 1) {
    freq[m[i, j], m[i, j + 1]] <- freq[m[i, j], m[i, j + 1]]  + 1 /  game_lengths[i]
  }
}
...
### Markov Chains Model (WIP, using matrix sub indexing is wrong)

```{r markov-chain-model}
d <- train_set %>% filter(as.logical(radiant_win)) %>% as_binned_matrix() %>% 
