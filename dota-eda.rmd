...
# create winners and losers transition matrices
winners_tm <- train_set %>%
  filter(as.logical(radiant_win)) %>%
  as_binned_matrix(nbin = 30) %>%
  construct_transition_matrix(nbin = 30) %>%
  matrix_to_df()
...
# plot the transition probablities for winners and losers
winners_tm %>%
