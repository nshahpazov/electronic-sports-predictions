...
```{r markov-pca}
winners_pca_tm <- train_set %>%
  filter(as.logical(radiant_win)) %>%
  pca_transform(cols = WINDOW_COLS[1:2]) %>%
...
# create losers transition matrix
losers_pca_tm <-  train_set %>%
  filter(!as.logical(radiant_win)) %>%
  pca_transform(WINDOW_COLS[1:2]) %>%
  as_binned_matrix(nbin = NBIN, feature = "pc") %>%
  construct_transition_matrix(nbin = NBIN)
```
...
* Tried PCA on different columns but maybe the calculations are off
